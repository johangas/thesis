...
#
# Getting the status of the Yanzi IoT-U10/IoT-U10+ button!
...
for data in d[1]:
  ...
  if instance > 0 and instance != i:
    True
  elif data[0] == tlvlib.BUTTON_GENERIC:
    found = 1
    t1 = tlvlib.createTLV(tlvlib.TLV_GET_REQUEST, i, tlvlib.VARIABLE_GPIO_INPUT, 0, None) 
    enc, tlvs = tlvlib.sendTLV([t1], host)
    if tlvs[0].Error == 0:
      buttonstatus = tlvs[0].IntValue
      if buttonstatus == 0:
...
if found == 0:
  print "Button instance not found"
